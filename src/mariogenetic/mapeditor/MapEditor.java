 ...
 * @author alice
 */
public class MapEditor extends JPanel implements KeyListener, MouseListener, MouseMotionListener{
    ...
    public void saveMap(String filename)
    {
        try {
            ...
            for(WorldObject wo : world_objects)
            {
                ...
                if(wo instanceof Terrain || wo instanceof Actor)
                {
                    WorldObject a = wo;
...
//        g2.drawString("Block Type: "+String.valueOf(selected_type), 10-screenDragTotal.x, 30-screenDragTotal.y);
     ...
     public static void main(String[] args) {
        ...
        final MapEditor map_main = new MapEditor();
        ...
        int loc_x = 0;
        if(GlobalVariables.main!=null)
        {
            loc_x = GlobalVariables.main.getSize().width;
            map_main.world_objects = GlobalVariables.main.resources.getReopenedResource();
         }
        else
        {
    ...
    public void mouseReleased(MouseEvent e) {
        if(e.getButton() == e.BUTTON1)
        {
            ...
            switch(selected_type)
            {
                ...
                case Player:
                {
                    world_objects.add(new Actor(new Vector(new_sel.x,new_sel.y),new Point(new_sel.width,new_sel.height)));
                    break;
                }
